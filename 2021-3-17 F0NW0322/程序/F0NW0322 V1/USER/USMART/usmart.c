...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{
		if(usmart_strcmp(sfname,(uint8_t *)sys_cmd_tab[i])==0)break;
	}
	switch(i)
	{					   
		case 0:
		case 1://帮助指令
			lcd_printf("\r\n");
#if USMART_USE_HELP
			lcd_printf("USMART有7个系统命令:\r\n");
			lcd_printf("?:      获取帮助信息\r\n");
			lcd_printf("help:   获取帮助信息\r\n");
			lcd_printf("list:   可用的函数列表\r\n\n");
			lcd_printf("id:     可用函数的ID列表\r\n\n");
			lcd_printf("hex:    参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
			lcd_printf("dec:    参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
			lcd_printf("runtime:1,开启函数运行计时;0,关闭函数运行计时;\r\n\n");
			lcd_printf("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");    
//			lcd_printf("--------------------------ALIENTEK------------------------- \r\n");
#else
			lcd_printf("指令失效\r\n");
#endif
		...
		case 2://查询指令
			lcd_printf("\r\n");
			lcd_printf("-------------------------函数清单--------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)lcd_printf("%s\r\n",usmart_dev.funs[i].name);
			lcd_printf("\r\n");
			break;	 
		case 3://查询ID
			lcd_printf("\r\n");
		...
		case 6://runtime指令,设置是否显示函数执行时间
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					if(USMART_ENTIMX_SCAN==0)lcd_printf("\r\nError! \r\nTo EN RunTime function,Please set USMART_ENTIMX_SCAN = 1 first!\r\n");//报错
					else
					{
						usmart_dev.runtimeflag=res;
						if(usmart_dev.runtimeflag)lcd_printf("Run Time Calculation ON\r\n");
						else lcd_printf("Run Time Calculation OFF\r\n"); 
					}
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			lcd_printf("%c",'"');			 
			lcd_printf("%s",usmart_dev.parm+usmart_get_parmpos(i));
			lcd_printf("%c",'"');
			temp[i]=(u32)&(usmart_dev.parm[usmart_get_parmpos(i)]);
		}else						  //参数是数字
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			if(usmart_dev.sptype==SP_TYPE_DEC)lcd_printf("%lu",temp[i]);//10进制参数显示
			else lcd_printf("0X%X",temp[i]);//16进制参数显示 	   
		}
		if(i!=pnum-1)lcd_printf(",");
	}
	lcd_printf(")");
	...
	if(rval==1)//需要返回值.
	{
		if(usmart_dev.sptype==SP_TYPE_DEC)lcd_printf("=%lu;\r\n",res);//输出执行结果(10进制参数显示)
		else lcd_printf("=0X%X;\r\n",res);//输出执行结果(16进制参数显示)	   
	}else lcd_printf(";\r\n");		//不需要返回值,直接输出结束
	if(usmart_dev.runtimeflag)	//需要显示函数执行时间
	{ 
		lcd_printf("Function Run Time:%d.%1dms\r\n",usmart_dev.runtime/10,usmart_dev.runtime%10);//打印函数执行时间 
	}	
}
#include "string.h"
//usmart扫描函数
...
void usmart_scan(void)
{
	u8 sta,len;  
	char buf[128];
	len = Read_USART_buffer(USART1,buf);
	if(len)//串口接收完成？
	{					   
		sta=usmart_dev.cmd_rec((uint8_t *)buf);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe((uint8_t *)buf);
...
#include "other.h"
#include "lcd3033.h"
unsigned char  Usmart_Debug_Fun(void){
		uart1_init(115200);
		Uart_printf (USART1 ,"USMART debug start...\n");
		usmart_dev.init(SystemCoreClock/1000000);	//???USMART	
		lcdreset ();
		lcdpos(1,0);
		lcd_printf  ("USMART bebug... ");
		usmart_dev.sptype = SP_TYPE_DEC;

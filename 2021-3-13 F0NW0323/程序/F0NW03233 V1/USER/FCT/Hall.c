...
#define Positive_IO  PFout(13)
#define Negative_IO  PFout(15)
#define HALL_Num_IO  PFout(14) 
...
/****************************************************************************************
  ...
  * return        : Test Result
	...
	* Date          : 24-June-2019
****************************************************************************************/
void Hall_TEST_Relay_SET(double REF_RES,uint8_t RATE){
...
/****************************************************************************************
  ...
  * return        : Test Result
	...
	* Date          : 24-June-2019
****************************************************************************************/
void Hall_TEST_Relay_RESET(void){
...
/*********************************************************************************************
	* File Name     : Set_ElectroMagnet
	* Description   : Set Set_ElectroMagnet 
  * Input         : Num
	* return        : ,
	* Version       : V1.0
	* Author        : Morton
	* Date          : 24-June-2019
*********************************************************************************************/
static void Set_ElectroMagnet( uint8_t Num, uint8_t Status ){
	uint8_t Hall_Pole = ( uint8_t )( Item[Num].IO_CTL&0x0F) ;
	uint8_t Hall_Num  = ( uint8_t )((Item[Num].IO_CTL>>4)&0x0F) ;
	...
	switch(Hall_Pole){
		case 0 : Positive_IO = 0; Negative_IO = 0;      break ;
		case 1 : Positive_IO = 1; Negative_IO = 0;      break;
		case 2 : Positive_IO = 0; Negative_IO = 1;      break;
		default :      break ;
	}
	
	switch(Hall_Num){
		case 1 : HALL_Num_IO = 0;      break;
		case 2 : HALL_Num_IO = 1;      break;
		default :      break ;
	}	
}
/*********************************************************************************************
	* File Name     : Read_Hall_Ouput_Vlots
	* Description   : HALL Sensor test function
  * Input         : ,
	* return        : Volts (V)
	* Version       : V1.0
	* Author        : Morton
	* Date          : 24-June-2019
*********************************************************************************************/
static  double  Read_Hall_Ouput_Vlots(uint16_t Num){

		double Vref = 0.0;
		Read_AD7176_Average_Voltage_mV(AIN2 ,AIN3, 25, &Vref);
...
/*********************************************************************************************
  ...
  * Input         : Num
	...
	* Date          : 24-June-2019
*********************************************************************************************/
uint8_t Hall_Test( uint8_t Num){
	uint16_t Test_Result = True,i,CNT = 0;
	double Volts,Vd[20];
	Set_ElectroMagnet(Num,SET );
	...
	if(Fixture.Mode  == Auto_Test_Mode ){
		for( i= 0 ; i < 300 ;i ++){			
				Volts  = Read_Hall_Ouput_Vlots(Num);
		...
		if(TestConfig.Hall  == ENABLE || Fixture.Mode  == Step_Test_Mode  ){
			cyc4or3run();
	...
	if(Fixture.Mode  == Step_Test_Mode  ){
			for( i = 0; i < 10; i ++){				
					Vd[i]  = Read_Hall_Ouput_Vlots(Num);
					delay_us (10);				
			}
			Item_Data[Num].Value  = Average_Fun(Vd,10);
			cyc4or3run ();
	...
	if(Item_Data[Num].Value > Item[Num].LSL && Item_Data[Num].Value < Item[Num].USL){
		...
		if(TestConfig.Hall  == DISABLE  && Fixture.Mode  == Auto_Test_Mode  ){
				cyc4or3run ();
...
/*********************************************************************************************
  ...
  * Input         : *TestStepN
	...
	* Date          : 13-May-2020
*********************************************************************************************/
u8 Type_Hall_Test_Fun(u16 *TestStepN, uint16_t  Total_Num,uint16_t *Step_Number_buffer){
		uint8_t k=0,Total_Test_Result = True;
		double  Range_RES[3] = {10,1000,20000};
		uint8_t Range_Type[3] = {Hall_10R,Hall_1Kohm,Hall_20Kohm};
		uint16_t j=0,range[3] = {0,0,0},Item_Step_Number = 0;

		if(Total_Num == 0)
			return True;
		Set_ElectroMagnet(0,RESET );
		if(TestConfig.Hall  == DISABLE &&Fixture.Mode  == Auto_Test_Mode ){
				(*TestStepN) ++;
				cyc4or3run ();
				lcd_printf ("%d>%s",*TestStepN,"HALL   ...   ");
		}	
		for( j = 0 ; j < Total_Num ; j ++){
			Item_Step_Number = Step_Number_buffer[j];
			if(Item[Item_Step_Number].Type == Hall_10R)
				range[0] ++;
			else if(Item[Item_Step_Number].Type == Hall_1Kohm)
				range[1] ++;
			else if(Item[Item_Step_Number].Type == Hall_20Kohm)
				range[2] ++;
		}
		for( k = 0; k < 3 ; k ++){
			if(range[k]){
				 Close_All_Point ();
				 Hall_TEST_Relay_SET(Range_RES[k],0);
				 for(j = 0 ; j < Total_Num ; j ++){
					 Item_Step_Number = Step_Number_buffer[j];
					 if(Item[Item_Step_Number].Type == Range_Type[k]){
							...
							Item_Data[Item_Step_Number].Status = Hall_Test(Item_Step_Number);		
		...
		if(TestConfig.Hall  == DISABLE &&Fixture.Mode  == Auto_Test_Mode && Total_Test_Result == True ){
				lcdpos(lcdcon -1,14);
				lcd_printf ("OK");
		}
		Set_ElectroMagnet(0,RESET );
		Hall_TEST_Relay_RESET();
		return Total_Test_Result ;	
}
